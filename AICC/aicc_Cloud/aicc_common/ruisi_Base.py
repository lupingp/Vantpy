# -*- coding: utf-8 -*-
# @Time    : 2018/11/23  10:05
# @Author  : 陆平！！
# @FileName: ruisi_Base.py
# @Software: PyCharm
...
class BasePage(object):
    ...
    def get_screent_img(self,name_=''):
        '''将页面截图下来'''
        file_path = os.path.dirname(os.getcwd()) + '\\aicc_image\\Screenshot__'
        now = time.strftime("_%Y-%m-%d_%H_%M_%S_")
        screen_name = file_path+name_+now+'.jpg'
        time.sleep(1)
        try:
            self.driver.get_screenshot_as_file(screen_name)
            logger.info("页面已截图，截图的路径在项目: /aicc_image路径下")
    ...
    def current_url(self,loc):
        self.driver.current_url(*loc)